...
class VerticalCardLayout: CardLayout {
    ...
    public var contentInset: UIEdgeInsets {
        ...
        get {
            return UIEdgeInsets(top: self.scrollView.contentInset.top, left: self.cardViewLeadingInset, bottom: self.scrollView.contentInset.bottom, right: self.cardViewTrailingInset)
        }
    }
    ...
    var presentationCenter: CGPoint {
        ...
        return self.scrollView.convert(CGPoint(x: centerRect.midX, y: centerRect.midY), from: self.budgetList!)
    ...
    func calculateLayoutValues(shouldLayoutView: Bool = true) {
        ...
        self.cardViewTopInset = self.scrollView.contentInset.top
    ...
    private func makeCollapseLayout(collapsePresentedCardView: Bool) {
        ...
        let scrollViewFrameMaxY = self.scrollView.convert(CGPoint(x: 0, y: scrollView.frame.maxY), from: budgetList).y
    ...
    private func placeVisibleCardViews() {
        ...
        let visibleScrollViewRect = CGRect(x: self.scrollView.contentOffset.x, y: self.scrollView.contentOffset.y, width: budgetList.frame.width, height: budgetList.frame.height)
